...
| Settings 粒度 | **per-channel `ClawImChannelV1.weixinStream`** | 跟随 `feishuStream` 在 commit `548baac`（2026-06-16）的 per-channel 重构；该 commit 把 `feishuStream` 从全局 `ClawImSettingsV1` 移到了 per-channel 字段 |
| 默认值 | **关闭**（`weixinStream: false`） | 与 `feishuStream` 一致（`548baac` 后 feishuStream 也默认关闭）；用户必须为每个微信 channel 显式启用流式 |
| UI 入口 | `settings-section-claw.tsx` 的 manage-agents 卡片，紧邻 `feishuStream` row，gate `channel.provider === 'weixin'` | 与 `feishuStream` row 平行，gate 由 `=== 'feishu'` 改为 `=== 'weixin'` |
| Migration | **不需要** | 与 `feishuStream` 一致 — 548baac 后旧全局 `claw.im.feishuStream` 字段被 `normalizeClawSettings` 静默丢弃；legacy 用户的"开"状态不会自动迁移过来（已知行为） |
...
### 2.5 `weixinStream` 设置（新增）— `src/shared/app-settings-types.ts` + `src/shared/app-settings-claw.ts`

```ts
// src/shared/app-settings-types.ts — 追加到 ClawImChannelV1 (紧邻 feishuStream)
interface ClawImChannelV1 {
  // ...已有字段（含 feishuStream?）
  /** 当 provider === 'weixin' 时,是否把 agent 回复改为 block streaming。默认 false (per-channel)。 */
  weixinStream?: boolean
}

// src/shared/app-settings-claw.ts — 在通道 .map() 回调的 feishuStream 后追加
//   normalizeClawSettings(channel mapper)
weixinStream: normalizeBoolean(raw.weixinStream, false)   // 默认 false,与 feishuStream 对齐
```

**关键设计**：
- **per-channel 字段**，与 `feishuStream` 完全同构（位置、命名、注释风格、默认值都镜像）
- 单一 boolean 字段；不做全局聚合（用户为每个微信 channel 单独启用）
- **没有 migration 代码**：旧全局 `claw.im.feishuStream` 字段在 548baac 之后被 `normalizeClawSettings` 静默丢弃；本 spec 沿用同一行为 — 升级用户需要为每个微信 channel 手动重新开启流式
- IPC schema `clawImChannelPatchSchema` 接受 `weixinStream: z.boolean().optional()`（紧邻 `feishuStream`）

### 2.6 Settings UI 入口（修改）— `src/renderer/src/components/settings-section-claw.tsx`

在飞书 `feishuStream` 现有的 `SettingRow` 旁边，加一行 `weixinStream` 切换（gate 由 `=== 'feishu'` 改为 `=== 'weixin'`）。

**关键设计**：
- 复用现有 `SettingRow` 组件和 inline 状态文字样式（飞书 spec 已确立的 pattern）
- gate `channel.provider === 'weixin'`，与 feishuStream row 平行
- 不引入新的 UI 抽象
- 调用方式：`updateChannel(form, update, channel.id, { weixinStream: value })`（与 feishuStream 完全一致；无需新 chat-store action）
...
在 `ClawImChannelV1`（per-channel）上加一个开关字段，紧邻 `feishuStream?`：
...
`src/shared/app-settings-claw.ts` 的通道 `.map()` 回调中追加 `weixinStream: normalizeBoolean(raw.weixinStream, false)`。

**IPC schema 同步**：在 `src/main/ipc/app-ipc-schemas.ts` 的 `clawImChannelPatchSchema`（约 line 586）追加 `weixinStream: z.boolean().optional()`（紧邻 `feishuStream`）。

**没有 migration**：与 `feishuStream` 一致。旧 settings 若曾把流式开关放在 `claw.im.weixinStream`（不存在的字段），被 `normalizeClawSettings` 静默丢弃。已知行为：升级用户的旧"开"状态不会自动迁移。

**Renderer**：在 `settings-section-claw.tsx` 飞书 row 紧邻处追加 weixinStream row，gate `channel.provider === 'weixin'`。
...
| 7 | `weixinStream` 默认值 | 新 channel（无 `weixinStream` 字段）normalize 后默认 `false`（与 feishuStream 对齐） |
...
| # | 用例 | 验证 |
|---|---|---|
| 1 | 渲染 `weixinStream` SettingRow | 默认 `false`（与 feishuStream 对齐），UI 渲染关闭态；gate `channel.provider === 'weixin'` 时可见 |
