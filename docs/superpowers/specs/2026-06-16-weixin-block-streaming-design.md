...
| Markdown | 复用 plugin 的 `StreamingMarkdownFilter` | plugin 已实现 char 级状态机；通过 `bridge-runtime` 的现有 import 路径加载 |
...
class WeixinStreamer {
  constructor(opts: {
    bridge: WeixinBridgeHandle        // 通过 weixin-bridge-runtime 暴露的 RPC 客户端
    accountId: string                 // 来自 webhook 载荷 message.accountId
    to: string                        // 来自 webhook 载荷 message.from
    turnId: string
    threadId: string
...
- **Markdown 过滤**：内部调用 plugin 的 `StreamingMarkdownFilter.feed(delta)` + `flush()`，得到可安全发给微信的文本，再喂给块合并器。
...
| 3 | `sendMessageWeixin` 抛错（rate_limited / permission_denied） | `runStreamingReplyWeixin` catch → 已发 block 数累计；若 ≥1 则补一条"未完成"；否则补"抱歉，生成失败" | catch 统一处理 |
...
| 7 | `sendMessageWeixin` 抛错（rate_limited） | 记 log；流继续；不立刻终止 |
